...
class ArticleSearchTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.es = NewsElasticEngine(initialize=True)

    def test_rebuilddatabase(self):
        result=self.es.rebuild_database(self.es.index_name,self.es.doc_type)
        self.assertTrue(result['acknowledged'])


    def test_articleInsert(self):
        ...
        for data in datas:
            res = self.es.insert_data(data)
            self.assertEqual(1, res['_shards']['successful'])
        
    def test_articleInsertandSearch(self):
        ...
        for data in datas:
            res = self.es.insert_data(data)
        time.sleep(3)
        self.assertGreaterEqual(json.loads(self.es.search_news("美国 中国 病毒"))['hits']['total']['value'],1)
